...
void eval(char *cmdline) 
{
  ...
  if(cmd2 != NULL)
  {
    ...
    if (pid1 == 0) 
    {
      ...
      {
        int newline_idx = strcspn(cmdline, "\n");
        cmdline[newline_idx] = '\0';
        printf(cmdline);
...
void do_bg(int jid) 
{
  ...
  if (kill(-pgid, SIGCONT) == -1)
    unix_error("couldn't foward message");
...
void waitfg(pid_t pid)
{ 
  // loop until pid isn't in the list of jobs
  sigset_t empty;
  sigemptyset(&empty);
  ...
  while(1)
  {
    ...
    // check if pid(fg job) in job list
    struct job_t* job = getjobpid(jobs, pid);
    if (job == NULL || job->state != FG)
      break;
...
void sigchld_handler(int sig) 
{
  ...
  while((finished_pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0)
  {
    ...
    if (WIFSTOPPED(status)) // (don't remove it)
    {
      int sig_num = WSTOPSIG(status);
      struct job_t* stopped_job = getjobpid(jobs, finished_pid);
    ...
    {
      struct job_t* cont_job = getjobpid(jobs, finished_pid);

      // check if do_fg or do_bg sent the signal
      if (cont_job->state == FG) 
        return;

      int jid = pid2jid(finished_pid);
      char* cmd = cont_job->cmdline;
